...
<head>
    ...
    <link rel="stylesheet"
        href="../assets/theme<?= $_POST['colors'] ?? $_COOKIE['colorsCookie'] ?? 'grey' ?>.css">
<body>
    <!-- =========================================== header -->
    ...
    <div class="row text-center m-5">
        <div class="col-sm text-dark mx-2 p-0">
          <h1><?= $titleFluxRSS[$_GET['subjectCookie'] ?? 0] ?></h2>
        </div>
      </div>

    <?php
        for ($row = 0; $row < count($fluxRSS[$_GET['subjectCookie'] ?? 0]->channel->item); $row++) {
    ?>

            <div class="card mb-3">
                <div class="row no-gutters">
                    <div class="col-md-1 bg-<?= $colorFluxRSS[$_GET['subjectCookie'] ?? 0] ?>">
                    </div>
                    <div class="col-sm-11">
                        <div class="card-body col-md-11">
                            <h5 class="card-title text-center"><?= $fluxRSS[$_GET['subjectCookie'] ?? 0]->channel->item[$row]->title ?></h5>
                            <p class="mb-0 text-center"><i class="fas fa-calendar mr-2"></i><?= strftime('%d/%m/%y à %Hh%M', strtotime(($fluxRSS[$_GET['subjectCookie'] ?? 0]->channel->item[$row])->pubDate)) ?></p>
                            <p class="card-text text-center"><?= $fluxRSS[$_GET['subjectCookie'] ?? 0]->channel->item[$row]->description ?>
                            <div class="text-right"><button class="bg-<?= $colorFluxRSS[$_GET['subjectCookie'] ?? 0] ?>"><a href="<?= $fluxRSS[$_GET['subjectCookie'] ?? 0]->channel->item[$row]->link ?>"
