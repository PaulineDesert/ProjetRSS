        ...
        <form action="" method="post" class="p-5 bg-light">
          ...
          <div class="form-group row  justify-content-center">
            ...
            <select id="colors" class="form-control col-sm-5" name="colors">
              <option value="grey"
                <?= (isset($_POST['colors'])) ? (($_POST['colors'] == 'grey')?'selected': '') : (isset($_COOKIE['colorsCookie'])? (($_COOKIE['colorsCookie'] == 'grey')?'selected':'selected'):'')?>>
                Thème gris</option>
              <option value="blue"
                <?= (isset($_POST['colors'])) ? (($_POST['colors'] == 'blue')?'selected': '') : (isset($_COOKIE['colorsCookie'])? (($_COOKIE['colorsCookie'] == 'blue')?'selected':''):'')?>>
                Thème Bleu</option>
              <option value="red"
                <?= (isset($_POST['colors'])) ? (($_POST['colors'] == 'red')?'selected': '') : (isset($_COOKIE['colorsCookie'])? (($_COOKIE['colorsCookie'] == 'red')?'selected':''):'')?>>
                Thème Rouge</option>
            </select>
          ...
          <div class="card m-2 bg-white">
            <div class="row">
              <div class="col-2 col-md-1 align-self-center">
                <div class="rounded d-block <?= $colorFluxRSS[$_COOKIE['subjectCookie' . $article]] ?>"
                  style="width:3rem; height: 6rem"></div>
              </div>
              ...
              <div class="col-1 p-0 align-self-center">
                <img src="assets/img/loup.svg" style="width:2rem" alt="Voir plus" title="Voir plus" data-toggle="modal"
                  data-target="#<?= 'id' . $col . $row?>">
              </div>
              <div class="col-1 p-0 align-self-center">
                <a href="<?= $fluxRSS[$_COOKIE['subjectCookie' . $article]]->channel->item[$row]->link ?>"
                  target="_blank">
                  <img src="assets/img/arrow.svg" style="width:2rem" alt="Aller vers l'article"
                    title="Aller vers l'article">
                </a>
              </div>
            </div>
          </div>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                ...
                <a href="<?= $fluxRSS[$_COOKIE['subjectCookie' . $article]]->channel->item[$row]->link ?>"
                  target="_blank" class="orange-text d-flex flex-row-reverse p-2">
